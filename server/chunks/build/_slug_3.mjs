...
import { C as Ct, z as zt, R as Rt, M as Mt, A as A$1, u, V, t } from '../nitro/nitro.mjs';
import { m } from './images-BZgZFbVl.mjs';
...
var C = ["<section", ' class="container" style="', '">', "</section>"], A = ["<h1", ">404</h1>"], H = ["<p", ">", "</p>"], M = ["<div", ' style="', '"><img', ' alt="', '" width="600" height="600" style="', '"><div><p class="eyebrow">', "</p><h1>", "</h1><p>", '</p><dl class="material-info">', '</dl><p style="', '">', "</p><p>", "</p></div></div>"], S = ["<div", ' style="', '"><dt style="', '">', '</dt><dd style="', '">', "</dd></div>"];
const T = V((l) => t(l), "material");
function K() {
  const l = Ct(), c = y(() => T(l.slug)), { t: a, lang: n } = zt();
  return ssr(C, ssrHydrationKey(), ssrStyleProperty("padding-block:", "var(--sp-5)"), escape(createComponent(Show, { get when() {
    ...
    return t().desc[n()];
  } }), ssr(M, ssrHydrationKey(), ssrStyleProperty("display:", "grid") + ssrStyleProperty(";grid-template-columns:", "repeat(auto-fit, minmax(min(20rem, 100%), 1fr))") + ssrStyleProperty(";gap:", "var(--sp-4)") + ssrStyleProperty(";align-items:", "center"), ssrAttribute("src", escape(m(t().slug), true), false), `Textura macro de ${escape(t().nombre, true)}`, ssrStyleProperty("border-radius:", "var(--radius)") + ssrStyleProperty(";width:", "100%") + ssrStyleProperty(";height:", "auto"), escape(a(`valores.tipo.${t().tipo}`)), escape(t().nombre), escape(t().desc[n()]), escape(createComponent(For, { get each() {
