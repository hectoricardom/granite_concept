...
import { I as I$1, k } from './index-BgYMpQL1.mjs';
import { Y as Yt, q as qe, X as Xe, C as Ce$1, P as Pe, K as Ke, G as Ge, M, n as ne, W as We, Z, k as ye$1, T as Te, l as Me } from '../nitro/nitro.mjs';
...
import { E, C } from './index-CctOUFvw.mjs';
...
const q = (e) => (r) => {
  const { base: n } = r, a = children(() => r.children), t = createMemo(() => qe(a(), r.base || ""));
  let s;
  const c = Xe(e, t, () => s, { base: n, singleFlight: r.singleFlight, transformUrl: r.transformUrl });
  return e.create && e.create(c), createComponent(Ce$1.Provider, { value: c, get children() {
...
function ue(e) {
  const r = e.routerState.location, n = e.routerState.params, a = createMemo(() => e.preload && untrack(() => {
    Te(true), e.preload({ params: n, location: r, intent: Ke() || "initial" }), Te(false);
  }));
  ...
  const a = createMemo(on(e.routerState.matches, (t, s, c) => {
    ...
    for (let d = 0, w = t.length; d < w; d++) {
      ...
      c && v && b.route.key === v.route.key ? m[d] = c[d] : (g = false, r[d] && r[d](), createRoot((L) => {
        r[d] = L, m[d] = Ge(e.routerState, m[d - 1] || e.routerState.base, I(() => a()[d + 1]), () => {
          var _a;
...
const I = (e) => () => createComponent(Show, { get when() {
  return e();
}, keyed: true, children: (r) => createComponent(ne.Provider, { value: r, get children() {
  return r.outlet();
} }) });
function de(e, r, n) {
  const a = new URL(e.request.url), t = M(n, new URL(e.router.previousUrl || e.request.url).pathname), s = M(n, a.pathname);
...
function ye(e) {
  ...
  return me({ get: r, set({ value: a, replace: t, scroll: s, state: c }) {
    t ? window.history.replaceState(We(c), "", a) : window.history.pushState(c, "", a), ge(decodeURIComponent(window.location.hash.slice(1)), s), Z();
  }, init: (a) => fe(window, "popstate", Me(a, (t) => {
...
var Ce = ["<main", ">", "</main>"];
function He() {
  return createComponent(ye, { root: (e) => createComponent(I$1, { get children() {
    return [createComponent(k, { children: "Granite Concepts \u2014 Encimeras de granito y cuarzo en Louisville, KY" }), createComponent(E, { get children() {
      return [createComponent(Se, {}), ssr(Ce, ssrHydrationKey(), escape(createComponent(Suspense, { get children() {
        return e.children;
      } }))), createComponent(Ae, {})];
    } })];
  } }), get children() {
    return createComponent(Yt, {});
  } });
}

export { He as default };
//# sourceMappingURL=app-BlMB62zI.mjs.map
