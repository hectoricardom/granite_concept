...
import { z as zt, M as Mt, A, x as xt, a, V, o, i } from '../nitro/nitro.mjs';
import { Show, For, createSignal, createEffect, on, onMount, createMemo, onCleanup } from 'solid-js';
import { m, h } from './images-BZgZFbVl.mjs';
...
function le(t) {
  const { t: a, lang: n } = zt(), s = t.available ? h(t.escena, t.material.slug) : "", [c, b] = createSignal(s), [o, d] = createSignal(""), [$, L] = createSignal("a"), [S, w] = createSignal(!t.available), [A, M] = createSignal(h$1()), [j, f] = createSignal("");
  let m$1 = 0;
  function h$1() {
...
function ve(t) {
  const { t: a, lang: n } = zt(), s = () => t.material, c = () => [[a("visualizador.tipo"), a(`valores.tipo.${s().tipo}`)], [a("visualizador.tono"), a(`valores.tono.${s().tono}`)], [a("visualizador.veta"), s().veta[n()]], [a("visualizador.dureza"), a(`valores.dureza.${s().dureza}`)], [a("visualizador.inversion"), "$".repeat(s().nivel)]];
...
function be(t) {
  const { t: a$1 } = zt(), [n] = xt(), s = createMemo(() => a.includes(n.escena) ? n.escena : "cocina"), c = createMemo(() => {
    var _a;
...
const ze = V(() => o(), "materials"), we = V(() => i(), "trabajos");
function ke() {
  const { t } = zt(), a = y(() => ze()), n = y(() => we());
  return [createComponent(Mt, { name: "description", content: "Encimeras de granito, cuarzo y m\xE1rmol en Louisville, KY. Visualiza cada piedra en tu cocina o ba\xF1o y pide tu cotizaci\xF3n gratis. Atenci\xF3n en espa\xF1ol e ingl\xE9s." }), createComponent(Mt, { property: "og:title", content: "Granite Concepts \u2014 Encimeras en Louisville, KY" }), createComponent(Mt, { property: "og:description", content: "Visualiza granito, cuarzo y m\xE1rmol en tu espacio. Cotizaci\xF3n gratis, atenci\xF3n biling\xFCe." }), createComponent(Mt, { property: "og:type", content: "website" }), createComponent(ae, {}), ssr($e, ssrHydrationKey(), escape(createComponent(Show, { get when() {
    return a();
