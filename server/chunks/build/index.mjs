...
import { J, F } from './index-Gy6Hlstv.mjs';
import { Show, For, onMount, createMemo, createSignal, createEffect, on, onCleanup } from 'solid-js';
import { c as nt, e as a, k, d as n, f as t } from '../nitro/nitro.mjs';
import { E, i } from './images-DUzp1oc32.mjs';
import { A } from './components-DxvOFfFE.mjs';
import { p } from './WorkCard-S6pLa_Ls.mjs';
...
function se(t) {
  const { t: r } = J(), l = i(t.escena, t.material.slug), [i$1, u] = createSignal(l), [c, o] = createSignal(""), [d, $] = createSignal("a"), [b, y] = createSignal(false), [g, x] = createSignal(L()), [A, G] = createSignal("");
  let w = 0;
  function L() {
  ...
  return createEffect(on(() => [t.escena, t.material.slug], ([p, Q], T) => {
    ...
    const M = i(p, Q), C = ++w, k = new Image();
    k.src = M, k.decode().then(() => {
      C === w && (y(false), d() === "a" ? (o(M), G(L()), $("b")) : (u(M), x(L()), $("a")));
    }).catch(() => {
      C === w && y(true);
    });
...
function me(t) {
  const { t: r, lang: l } = J(), i = () => [[r("visualizador.tipo"), t.material.tipo], [r("visualizador.tono"), t.material.tono], [r("visualizador.veta"), t.material.veta], [r("visualizador.dureza"), t.material.dureza.replace("-", " ")], [r("visualizador.inversion"), "$".repeat(t.material.nivel)]];
...
function ge(t) {
  const { t: r } = J(), [l] = nt(), i = createMemo(() => a.includes(l.escena) ? l.escena : "cocina"), u = createMemo(() => {
    var _a;
