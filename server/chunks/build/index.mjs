...
import { H } from './index-BgYMpQL1.mjs';
import { Show, For, createSignal, createEffect, on, onMount, createMemo, onCleanup } from 'solid-js';
import { e as He, m, f as p, g as h, i as y$1 } from '../nitro/nitro.mjs';
import { C } from './index-CctOUFvw.mjs';
import { A } from './components-CDQikSFO.mjs';
import { B, a } from './WorkCard-COqmfKzm.mjs';
...
function pe(t) {
  const { t: a, lang: d } = C(), n = () => t.material, c = () => [[a("visualizador.tipo"), a(`valores.tipo.${n().tipo}`)], [a("visualizador.tono"), a(`valores.tono.${n().tono}`)], [a("visualizador.veta"), n().veta[d()]], [a("visualizador.dureza"), a(`valores.dureza.${n().dureza}`)], [a("visualizador.inversion"), "$".repeat(n().nivel)]];
...
function $e(t) {
  const { t: a$1 } = C(), [d] = He(), n = createMemo(() => t.scenes && t.scenes.length ? t.scenes : a.map((s) => ({ slug: s, nombre: a$1(`visualizador.escenas.${s}`) }))), c = createMemo(() => {
    const s = n();
...
const ze = m(() => p(), "materials"), Se = m(() => h(), "scenes"), _e = m(() => y$1(), "trabajos");
function Ie() {
  const { t } = C(), a = y(() => ze()), d = y(() => Se()), n = y(() => _e());
  return [createComponent(H, { name: "description", content: "Encimeras de granito, cuarzo y m\xE1rmol en Louisville, KY. Visualiza cada piedra en tu cocina o ba\xF1o y pide tu cotizaci\xF3n gratis. Atenci\xF3n en espa\xF1ol e ingl\xE9s." }), createComponent(H, { property: "og:title", content: "Granite Concepts \u2014 Encimeras en Louisville, KY" }), createComponent(H, { property: "og:description", content: "Visualiza granito, cuarzo y m\xE1rmol en tu espacio. Cotizaci\xF3n gratis, atenci\xF3n biling\xFCe." }), createComponent(H, { property: "og:type", content: "website" }), createComponent(re, { get materials() {
    return a();
