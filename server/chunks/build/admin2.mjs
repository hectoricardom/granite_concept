...
import { a as at, n as nt, r as rt, H as H$1, Z as Z$1 } from './auth-C4c1oFqj.mjs';
import 'node:http';
import 'node:https';
import 'node:events';
import 'node:buffer';
...
function Se() {
  const n = He(), [a, m] = createSignal(false), [t, d] = createSignal(null), [o, $e] = createSignal("materiales"), [T, _] = createSignal(null);
  onMount(async () => {
    if (await at(), !nt() || !rt()) {
  ...
  const $ = O();
  return ssr(G, ssrHydrationKey(), escape(createComponent(ht, { children: "Panel \u2014 Granite Concepts" })), escape(createComponent(Show, { get when() {
    return a();
  }, get children() {
